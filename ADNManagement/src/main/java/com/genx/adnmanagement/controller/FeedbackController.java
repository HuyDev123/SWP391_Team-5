...
public class FeedbackController {
    ...
    public class FeedbackApiController {
        ...
        // Cập nhật feedback
        @PutMapping("/update")
        public ResponseEntity<?> updateFeedback(@Valid @RequestBody FeedbackRequest request, HttpSession session) {
            try {
                // Lấy user từ session hoặc từ request (cho trường hợp iframe)
                User user = null;
                Integer userId = (Integer) session.getAttribute("userId");
                ...
                Optional<Feedback> existingFeedbackOpt = feedbackRepository.findByBookingIdAndServiceId(
                    request.getBookingId(), request.getServiceId());
                
                if (existingFeedbackOpt.isEmpty()) {
                    return ResponseEntity.badRequest().body(Map.of("message", "Không tìm thấy feedback để cập nhật"));
                }

                Feedback existingFeedback = existingFeedbackOpt.get();
                
                // Kiểm tra xem user có quyền cập nhật feedback này không
                if (!existingFeedback.getUser().getId().equals(user.getId())) {
                    return ResponseEntity.status(403).body(Map.of("message", "Bạn không có quyền cập nhật feedback này"));
                }

                // Cập nhật thông tin feedback
                existingFeedback.setRating(request.getRating());
                existingFeedback.setComment(request.getComment() != null ? request.getComment().trim() : null);

                Feedback updatedFeedback = feedbackRepository.save(existingFeedback);

                Map<String, Object> response = new HashMap<>();
                response.put("message", "Cập nhật feedback thành công");
                response.put("feedbackId", updatedFeedback.getId());
                response.put("bookingId", request.getBookingId());
                response.put("serviceId", request.getServiceId());
                response.put("userId", user.getId());
                ...
                return ResponseEntity.internalServerError().body(Map.of("message", "Lỗi khi cập nhật feedback: " + e.getMessage()));
            }
        }
    }
} 