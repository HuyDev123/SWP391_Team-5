...
public class KitController {
    ...
    public ResponseEntity<?> getKits(
        ...
        try {
            ...
            // Log tham số truyền vào
            System.out.println("[KitController] /api/kits/staff params: search=" + search + ", status=" + status + ", page=" + page + ", size=" + size);
            ...
            // Log tổng số booking tìm được
            System.out.println("[KitController] /api/kits/staff total bookings found: " + bookingPage.getTotalElements());
            for (Booking booking : bookingPage.getContent()) {
                System.out.println("[KitController] Booking: id=" + booking.getId() + ", status=" + booking.getStatus() + ", kitStatus=" + booking.getKitStatus() + ", isCenterCollected=" + booking.getIsCenterCollected());
            }
    ...
    public ResponseEntity<?> sendKits(@PathVariable Integer appointmentId, @RequestBody Map<String, Object> payload) {
        try {
            ...
            try {
                ...
                System.err.println("[KitController] Lỗi gửi email thông báo gửi kit: " + e.getMessage());
            }
    ...
    public ResponseEntity<?> checkSendKitPayment(@PathVariable Integer appointmentId) {
        ...
        List<BookingService> bookingServices = bookingServiceRepository.findByBookingId(appointmentId);
        double totalAmount = bookingServices.stream().mapToDouble(bs -> bs.getService().getPrice().doubleValue()).sum();
        double totalPaid = paymentRepository.findByBooking_Id(appointmentId).stream().mapToDouble(p -> p.getAmount().doubleValue()).sum();
        double minRequired = Math.ceil(totalAmount * 0.1);
        double needToPay = minRequired - totalPaid;
        ...
        if (totalPaid < minRequired) {
            ...
            if (hasPendingPayment) {
                resp.put("message", String.format("Không thể gửi kit. Khách hàng chưa thanh toán đủ tối thiểu 10%%. Có thanh toán cần xử lý. Đã thanh toán: %.0f VND / Tối thiểu: %.0f VND (Thiếu: %.0f VND)", totalPaid, minRequired, needToPay));
            } else {
                resp.put("message", String.format("Không thể gửi kit. Khách hàng chưa thanh toán đủ tối thiểu 10%%. Đã thanh toán: %.0f VND / Tối thiểu: %.0f VND (Thiếu: %.0f VND)", totalPaid, minRequired, needToPay));
            }
    ...
    public ResponseEntity<?> checkReceiveSamplePayment(@PathVariable Integer appointmentId) {
        ...
        double totalAmount = bookingServices.stream().mapToDouble(bs -> bs.getService().getPrice().doubleValue()).sum();
        double totalPaid = paymentRepository.findByBooking_Id(appointmentId).stream().mapToDouble(p -> p.getAmount().doubleValue()).sum();
        double needToPay = totalAmount - totalPaid;
        List<Payment> payments = paymentRepository.findByBooking_Id(appointmentId);
        boolean hasPendingPayment = payments.stream().anyMatch(p -> p.getAmount() != null && p.getAmount().doubleValue() == 0);
        if (totalPaid < totalAmount) {
            ...
            resp.put("hasPendingPayment", hasPendingPayment);
            if (hasPendingPayment) {
                resp.put("message", String.format("Không thể cập nhật trạng thái 'Đã nhận mẫu'. Khách hàng chưa thanh toán đủ. Có thanh toán cần xử lý. Đã thanh toán: %.0f VND / Tổng tiền: %.0f VND (Thiếu: %.0f VND)", totalPaid, totalAmount, needToPay));
            } else {
                resp.put("message", String.format("Không thể cập nhật trạng thái 'Đã nhận mẫu'. Khách hàng chưa thanh toán đủ. Đã thanh toán: %.0f VND / Tổng tiền: %.0f VND (Thiếu: %.0f VND)", totalPaid, totalAmount, needToPay));
            }
            return ResponseEntity.ok(resp);
        }
        return ResponseEntity.ok(Map.of("ok", true));
    ...
    public ResponseEntity<?> remind10PercentPayment(@PathVariable Integer appointmentId) {
        try {
            ...
            List<BookingService> bookingServices = bookingServiceRepository.findByBookingId(appointmentId);
            double totalAmount = bookingServices.stream().mapToDouble(bs -> bs.getService().getPrice().doubleValue()).sum();
            double totalPaid = paymentRepository.findByBooking_Id(appointmentId).stream().mapToDouble(payment -> payment.getAmount().doubleValue()).sum();
            double minRequired = Math.ceil(totalAmount * 0.1);
            double needToPay = minRequired - totalPaid;
            if (needToPay <= 0) {
            ...
            try {
                ...
                System.err.println("[KitController] Lỗi gửi email nhắc thanh toán 10%: " + e.getMessage());
                return ResponseEntity.ok("Đã thực hiện yêu cầu nhắc thanh toán (email có thể gặp sự cố)");
            }
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<?> remindFullPayment(@PathVariable Integer appointmentId) {
        try {
            ...
            try {
                ...
                System.err.println("[KitController] Lỗi gửi email nhắc thanh toán 100%: " + e.getMessage());
                return ResponseEntity.ok("Đã thực hiện yêu cầu nhắc thanh toán (email có thể gặp sự cố)");
            }
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
