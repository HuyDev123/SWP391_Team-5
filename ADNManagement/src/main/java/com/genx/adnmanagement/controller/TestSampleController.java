...
public class TestSampleController {
    ...
    public ResponseEntity<?> getAllTestSamples() {
        try {
            System.out.println("Getting all test samples...");
            List<TestSample> testSamples = testSampleRepository.findAll();
            System.out.println("Found " + testSamples.size() + " test samples");
            ...
            return ResponseEntity.ok(testSampleDTOs);
        } catch (Exception e) {
    ...
    @GetMapping("/search")
    public ResponseEntity<?> searchTestSamples(@RequestParam(required = false) String searchTerm) {
        try {
            List<TestSample> testSamples;
            ...
            if (searchTerm == null || searchTerm.trim().isEmpty()) {
                ...
                testSamples = testSampleRepository.findAll();
                System.out.println("Loading all test samples: " + testSamples.size());
            } else {
                ...
                try {
                    ...
                    if (sampleOpt.isPresent()) {
                        testSamples = List.of(sampleOpt.get());
                        ...
                        testSamples = testSampleRepository.findByParticipantId(numericValue);
                        System.out.println("Found " + testSamples.size() + " test samples by participant ID: " + numericValue);
                    }
                } catch (NumberFormatException e) {
                    // Nếu không phải số, tìm kiếm theo text
                    testSamples = testSampleRepository.searchTestSamplesByText(trimmedTerm);
                    System.out.println("Found " + testSamples.size() + " test samples by text search: " + trimmedTerm);
                }
                
                // Loại bỏ trùng lặp dựa trên ID của test sample
                testSamples = testSamples.stream()
                    .distinct()
                    .toList();
            }
