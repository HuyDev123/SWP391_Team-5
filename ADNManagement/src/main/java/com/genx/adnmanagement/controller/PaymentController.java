...
public class PaymentController {
    ...
    public ResponseEntity<?> uploadReceipt(@RequestParam("receipt") MultipartFile file,
        ...
        try {
            ...
            // Tạo payment mới với amount = 0, paymentMethod = BANK_TRANSFER
            ...
            payment.setReceiptImage(imagePath);
            payment.setStaff(null);
            payment.setNote(null);
            Payment savedPayment = paymentRepository.save(payment);
            Map<String, Object> response = new HashMap<>();
            response.put("message", "Đã upload ảnh và tạo yêu cầu thanh toán thành công");
            response.put("paymentId", savedPayment.getId());
            response.put("createdAt", savedPayment.getCreatedAt());
            response.put("receiptImage", savedPayment.getReceiptImage());
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Lỗi khi upload ảnh và tạo thanh toán: " + e.getMessage());
        }
    }
    ...
    @PostMapping(value = "/staff/create-with-receipt", consumes = "multipart/form-data")
    public ResponseEntity<?> createPaymentWithReceiptByStaff(
            @RequestParam("bookingId") Integer bookingId,
        ...
        try {
            ...
            if (receiptImage != null && !receiptImage.isEmpty()) {
                ...
                // Lưu file
                Path filePath = uploadPath.resolve(filename);
                Files.copy(receiptImage.getInputStream(), filePath);
            ...
            payment.setNote(notes);
            payment.setCreatedAt(LocalDateTime.now());
            if (receiptPath != null) payment.setReceiptImage(receiptPath);
            Payment savedPayment = paymentRepository.save(payment);
            Map<String, Object> response = new HashMap<>();
            response.put("message", "Tạo thanh toán kèm ảnh thành công");
            response.put("paymentId", savedPayment.getId());
            response.put("amount", savedPayment.getAmount());
            response.put("paymentMethod", savedPayment.getPaymentMethod());
            response.put("createdAt", savedPayment.getCreatedAt());
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Lỗi khi tạo thanh toán kèm ảnh: " + e.getMessage());
        }
    }
