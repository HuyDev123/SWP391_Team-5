...
const TEST_SAMPLES_API = {
    ...
    getAllTestSamples: async function() {
        try {
    ...
    // Search test samples
    searchTestSamples: async function(searchTerm) {
        try {
            const response = await fetch(`/test-samples/search?searchTerm=${encodeURIComponent(searchTerm)}`);
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
            return await response.json();
