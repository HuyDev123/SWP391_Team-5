    ...
    <div class="modal-content" style="max-width: 700px;">
      <div class="modal-header">
        <h2 class="modal-title">${
          options.title || "Nhập Thông Tin Người Tham Gia"
        }</h2>
      ...
      <form id="global-participant-form">
        <div class="form-grid">
          <div class="form-group">
            ...
            <input type="text" id="gp-fullName" name="fullName" class="form-control" required value="${
              options.fullName || ""
            }">
          </div>
          <div class="form-group">
            <label for="gp-phone">Số điện thoại <span class="required">*</span></label>
            <input type="tel" id="gp-phone" name="phone" class="form-control" required value="${
              options.phone || ""
            }">
          </div>
          <div class="form-group">
            <label for="gp-email">Email <span class="required">*</span></label>
            <input type="email" id="gp-email" name="email" class="form-control" required value="${
              options.email || ""
            }">
          </div>
          <div class="form-group">
            <label for="gp-address">Địa chỉ <span class="required">*</span></label>
            <input type="text" id="gp-address" name="address" class="form-control" required value="${
              options.address || ""
            }">
          </div>
          <div class="form-group">
            <label for="gp-birthDate">Ngày sinh <span class="required">*</span></label>
            <input type="date" id="gp-birthDate" name="birthDate" class="form-control" required value="${
              options.birthDate || ""
            }">
          </div>
          <div class="form-group">
            ...
            <select id="gp-gender" name="gender" class="form-control" required>
              ...
              <option value="male" ${
                options.gender === "male" ? "selected" : ""
              }>Nam</option>
              <option value="female" ${
                options.gender === "female" ? "selected" : ""
              }>Nữ</option>
              <option value="other" ${
                options.gender === "other" ? "selected" : ""
              }>Khác</option>
            </select>
          </div>
          <div class="form-group">
            <label for="gp-relationship">Mối quan hệ <span class="required">*</span></label>
            <input type="text" id="gp-relationship" name="relationship" class="form-control" required value="${
              options.relationship || ""
            }">
          </div>
          <div class="form-group">
            <label for="gp-note">Ghi chú</label>
            <textarea id="gp-note" name="note" class="form-control" rows="2">${
              options.note || ""
            }</textarea>
          </div>
        </div>
...
function openSampleModal(options = {}) {
  ...
  if (
    Array.isArray(options.participantList) &&
    options.participantList.length > 0
  ) {
    participantField = `
    ...
    participantField = `
      ...
      <input type="text" id="gs-participant" name="participant" class="form-control" required value="${
        options.participant || ""
      }">
    `;
  }
  ...
  if (Array.isArray(options.staffList) && options.staffList.length > 0) {
    ...
    collectedByField = `
      <label for="gs-collectedBy">Người thực hiện <span class="required">*</span></label>
      <input type="text" id="gs-collectedBy" name="collectedBy" class="form-control" required value="${
        options.collectedBy || ""
      }">
    `;
  }
  ...
  if (Array.isArray(options.serviceList) && options.serviceList.length > 0) {
    ...
    serviceField = `
      <label for="gs-service">Dịch vụ <span class="required">*</span></label>
      <input type="text" id="gs-service" name="service" class="form-control" required value="${
        options.service || ""
      }">
    `;
  }
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 700px;">
      <div class="modal-header">
        <h2 class="modal-title">${
          options.title || "Nhập Thông Tin Mẫu Xét Nghiệm"
        }</h2>
      ...
      <form id="global-sample-form">
        <div class="form-grid">
          <div class="form-group">
            ...
            <input type="text" id="gs-sampleCode" name="sampleCode" class="form-control" required value="${
              options.sampleCode || ""
            }">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="datetime-local" id="gs-collectedAt" name="collectedAt" class="form-control" required value="${
              options.collectedAt || ""
            }">
          </div>
          <div class="form-group">
            ...
            <select id="gs-sampleType" name="sampleType" class="form-control" required>
              ...
              <option value="blood" ${
                options.sampleType === "blood" ? "selected" : ""
              }>Máu</option>
              <option value="saliva" ${
                options.sampleType === "saliva" ? "selected" : ""
              }>Nước bọt</option>
              <option value="hair" ${
                options.sampleType === "hair" ? "selected" : ""
              }>Tóc</option>
              <option value="nail" ${
                options.sampleType === "nail" ? "selected" : ""
              }>Móng tay</option>
            </select>
          </div>
          <div class="form-group">
            <label for="gs-note">Ghi chú</label>
            <textarea id="gs-note" name="note" class="form-control" rows="2">${
              options.note || ""
            }</textarea>
          </div>
        </div>
...
async function checkAndHandleSampleCollection(bookingId) {
  try {
    ...
    const nextService = (status.services || []).find(
      (s) => s.sampleQuantityActual < s.sampleQuantityRequired
    );
    if (nextService) {
      // Mở modal thêm mẫu cho dịch vụ này
      // Lấy danh sách participant thực tế từ booking
      const partRes = await fetch(
        `/participants/booking/${bookingId}/participants`
      );
      ...
      const serviceRes = await fetch(
        `/participants/booking/${bookingId}/services`
      );
    ...
    updateBookingStatusSampled(bookingId, () => {
      if (typeof window.showSuccessToast === "function")
        window.showSuccessToast('Đã cập nhật trạng thái thành "Đã lấy mẫu"!');
...
async function updateBookingStatusSampled(bookingId, callback) {
  try {
    // Lấy thông tin booking để kiểm tra phương thức thanh toán
    const bookingRes = await fetch(`/booking/${bookingId}`);
    if (!bookingRes.ok) throw new Error(await bookingRes.text());
    const booking = await bookingRes.json();
    // Nếu là tại nhà (COD), kiểm tra thanh toán đủ chưa
    if (booking.method === "Tại nhà" && booking.paymentMethod === "COD") {
      // Gọi API kiểm tra thanh toán
      const payRes = await fetch(`/booking/${bookingId}/check-payment`);
      if (!payRes.ok) throw new Error(await payRes.text());
      const payStatus = await payRes.json();
      if (!payStatus.paidEnough) {
        // Nếu chưa đủ tiền, mở modal thêm thanh toán
        openAddPaymentModal({
          booking: {
            id: bookingId,
            customerName: booking.customerName,
            remainingAmount: payStatus.remainingAmount,
            paymentMethod: booking.paymentMethod,
            method: booking.method,
          },
          onSubmit: () => {
            // Sau khi thêm thanh toán, kiểm tra lại
            updateBookingStatusSampled(bookingId, callback);
          },
        });
        return;
      }
    }
    // Nếu đã đủ tiền hoặc không phải COD, cập nhật trạng thái như cũ
...
function openAddPaymentModal(options = {}) {
  ...
  const formattedAmount = new Intl.NumberFormat("vi-VN", {
    style: "currency",
    currency: "VND",
  }).format(booking.remainingAmount || 0);
    ...
    <div class="modal-content" style="max-width:600px;">
      ...
      <form id="global-add-payment-form">
        <div style="background:#f5f5f5;padding:15px;border-radius:5px;margin-bottom:18px;">
          <div style="margin-bottom:8px;"><b>Mã lịch hẹn:</b> <span>${
            booking.id || ""
          }</span></div>
          <div style="margin-bottom:8px;"><b>Khách hàng:</b> <span>${
        ...
        <div class="form-group" style="margin-bottom:15px;">
          ...
          <input type="number" id="ap-amount" name="amount" class="form-control" required min="1" value="${
  ...
  // Submit event
  document.getElementById("global-add-payment-form").onsubmit = async function (
    e
  ) {
    e.preventDefault();
    ...
    try {
      ...
      if (receiptFile) {
        ...
        paymentResult = await STAFF_API.PAYMENT.createPaymentWithReceipt(
          formData
        );
      } else {
        paymentResult = await STAFF_API.PAYMENT.createPayment(
          booking.id,
          amount,
          paymentMethod,
          notes
      ...
      const res = await fetch(
        `/booking/staff/appointments/${booking.id}/status`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (typeof options.onSubmit === "function") {
        ...
        if (typeof window.showSuccessToast === "function")
          window.showSuccessToast(
            "Tạo thanh toán và cập nhật trạng thái thành công!"
          );
        modal.remove();
      }
    } catch (err) {
      if (typeof window.showErrorToast === "function")
        window.showErrorToast(
          "Lỗi khi tạo thanh toán hoặc cập nhật trạng thái: " + err.message
        );
    }
  };
}
...
function openResultModalUniversal(options = {}) {
  ...
  function renderServiceList() {
    ...
    const allCompleted =
      services.length > 0 && services.every((s) => !!s.resultFile);
        ...
        <div style="display:flex;flex-direction:column;gap:12px;">
          ${services
            ...
            <div style="display:flex;align-items:center;gap:16px;background:#f8f9fa;padding:12px 18px;border-radius:10px;">
              ...
              ${
                s.resultFile
                  ? `<span style='font-size:13px;color:#666;margin-left:10px;'><i class='fas fa-file-pdf' style='color:#dc3545;'></i> ${s.resultFile}</span>`
                  : ""
              }
            </div>
          `
  ...
  function renderServiceForm(idx) {
    ...
    return `
      ...
      <div class="form-actions">
        ...
        <button type="button" class="btn btn-primary" id="result-submit-btn"><i class="fas fa-save"></i> ${
          s.resultFile ? "Cập nhật" : "Lưu kết quả"
        }</button>
      </div>
    `;
  }
        ...
        <div id="result-modal-body">
          ${
            selectedServiceIdx === null
              ? renderServiceList()
              : renderServiceForm(selectedServiceIdx)
          }
        </div>
      </div>
    `;
    ...
    setTimeout(() => {
      ...
      if (btnReturn) {
        btnReturn.onclick = async function () {
          ...
          btnReturn.innerHTML =
            '<i class="fas fa-spinner fa-spin"></i> Đang trả kết quả...';
          try {
            const res = await fetch(
              `/booking/staff/appointments/${bookingId}/status`,
              {
            ...
            if (!res.ok) throw new Error(await res.text());
            if (typeof window.showSuccessToast === "function")
              window.showSuccessToast("Đã trả kết quả thành công!");
            modal.remove();
            ...
            if (typeof window.showErrorToast === "function")
              window.showErrorToast("Lỗi khi trả kết quả: " + err.message);
  ...
  function setupFileUpload() {
    ...
    function handleFiles(files) {
      ...
      selectedFile = file;
      fileList.innerHTML = `<div class='file-item'><i class='fas fa-file-pdf' style='color:#dc3545;'></i> <span>${
        file.name
      }</span> <small style='color:#6c757d;margin-left:10px;'>(${(
        file.size /
        1024 /
        1024
      ).toFixed(
        2
      )} MB)</small> <button type='button' class='file-remove' onclick='window._removeResultFile()' title='Xóa file'>×</button></div>`;
    }
  ...
  function openPdfViewerModal(fileName, title = "Xem kết quả") {
    ...
    setTimeout(() => {
      ...
      if (loading && iframe.style.display === "none") {
        loading.innerHTML =
          '<i class="fas fa-exclamation-triangle"></i> Không thể tải file PDF. Vui lòng thử lại.';
  ...
  async function submitResult() {
    ...
    try {
      ...
      if (typeof window.showSuccessToast === "function") {
        window.showSuccessToast(
          `Đã lưu kết quả thành công! File: ${result.fileName}`
        );
      } else {
