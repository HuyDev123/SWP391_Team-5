...
function openSampleModal(options = {}) {
  ...
  // Nếu có serviceName thì chỉ hiển thị text, không cho chọn lại
  let serviceField = '';
  if (options.serviceName) {
    serviceField = `
      <label for="gs-service">Dịch vụ <span class="required">*</span></label>
      <input type="text" id="gs-service" name="service" class="form-control" required value="${options.serviceName}" readonly>
      <input type="hidden" id="gs-service-id" value="${options.selectedServiceId || ''}">
    `;
  } else if (Array.isArray(options.serviceList) && options.serviceList.length > 0) {
  ...
  if (typeof options.onSubmit === 'function') {
    document.getElementById('global-sample-form').onsubmit = function(e) {
      ...
      let serviceValue = '';
      if (options.serviceName) {
        serviceValue = document.getElementById('gs-service-id').value;
      } else {
        serviceValue = document.getElementById('gs-service').value;
      }
      const data = {
...
// Hàm dùng chung cho cả 2 trang: nhập participant & sample, updateType quyết định cập nhật trạng thái cuối
async function checkAndHandleSampleInput(bookingId, updateType) {
  try {
    ...
    if (status.numParticipantsActual < status.numParticipantsRequired) {
      ...
      const current = status.numParticipantsActual + 1;
      const total = status.numParticipantsRequired;
      openParticipantModal({
        title: `Nhập thông tin người tham gia (${current}/${total})`,
        onSubmit: async (participantData, modal) => {
          try {
            ...
            checkAndHandleSampleInput(bookingId, updateType);
          } catch (err) {
    ...
    if (nextService) {
      ...
      // Lấy danh sách services của booking để lấy tên dịch vụ chính xác
      ...
      // Tìm tên dịch vụ chính xác từ serviceList dựa trên serviceId
      let serviceName = "";
      if (nextService.serviceId) {
        const serviceInfo = serviceList.find(s => s.id === nextService.serviceId);
        if (serviceInfo && serviceInfo.name) {
          serviceName = serviceInfo.name;
        }
      }

      // Tính toán current và total cho tiêu đề
      const current = nextService.sampleQuantityActual + 1 || 1;
      const total = nextService.sampleQuantityRequired || 1;

      openSampleModal({
        title: `Nhập mẫu cho dịch vụ ${serviceName} (mẫu ${current}/${total})`,
        participantList: Array.isArray(partList) ? partList : [],
        staffList: Array.isArray(staffList) ? staffList : [],
        serviceName: serviceName,
        selectedServiceId: nextService.serviceId || nextService.service,
    ...
    // Đủ participant & sample, cập nhật trạng thái cuối cùng
    if (updateType === 'kit') {
      doUpdateKitStatus(bookingId, 'Đã nhận mẫu');
    } else {
...
async function updateBookingStatusSampled(bookingId, callback) {
  ...
  if (typeof window.showConfirmModal === 'function') {
    window.showConfirmModal(
      'Xác nhận lấy mẫu',
      'Bạn có chắc chắn muốn cập nhật trạng thái lịch hẹn này sang <b>Đã lấy mẫu</b>?',
      async () => {
        await doUpdateBookingStatusSampled(bookingId, callback);
      }
    );
  } else {
    // Nếu không có modal xác nhận, thực hiện luôn
    await doUpdateBookingStatusSampled(bookingId, callback);
  }
}

// Hàm thực hiện cập nhật trạng thái thật sự
async function doUpdateBookingStatusSampled(bookingId, callback) {
  try {
...
async function openAddPaymentModal(options = {}) {
  // Xóa modal cũ nếu có
  const oldModal = document.getElementById('global-add-payment-modal');
  if (oldModal) oldModal.remove();

  // Lấy bookingId từ options
  const bookingId = options.bookingId || (options.booking && options.booking.id);
  if (!bookingId) {
    alert('Không tìm thấy mã lịch hẹn!');
    return;
  }

  // Hàm fetch chi tiết booking từ backend
  async function fetchBookingPaymentInfo(bookingId) {
    const res = await fetch(`/booking/staff/appointments/${bookingId}`, { credentials: 'include' });
    if (!res.ok) throw new Error(await res.text());
  ...
  let booking;
  try {
    booking = await fetchBookingPaymentInfo(bookingId);
  } catch (err) {
    alert('Lỗi khi lấy thông tin lịch hẹn: ' + err.message);
    return;
  }

  // booking: { id, customerName, remainingAmount, paymentMethod, method, totalAmount, paidAmount }
    ...
    <div class="modal-content" style="max-width:600px; max-height: 80vh; overflow-y: auto;">
      ...
      <form id="global-add-payment-form">
        <div style="background:#f5f5f5;padding:15px;border-radius:5px;margin-bottom:18px;">
          ...
          <div style="margin-bottom:8px;"><b>Tổng tiền:</b> <span>${new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(booking.totalAmount || 0)}</span></div>
          <div style="margin-bottom:8px;"><b>Đã thanh toán:</b> <span>${new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(booking.paidAmount || 0)}</span></div>
  ...
  document.getElementById('global-add-payment-form').onsubmit = async function(e) {
    ...
    try {
      ...
      if (typeof options.onSubmit === 'function') {
        ...
        if (typeof window.showSuccessToast === 'function') window.showSuccessToast('Thêm thanh toán thành công!');
      }
    } catch (err) {
  ...
  function renderModalContent() {
    modal.innerHTML = `
      <div class="modal-content" style="max-width:650px; max-height: 80vh; overflow-y: auto;">
        ...
        <div id="result-modal-body" class="modal-body" style="padding: 24px 30px;">
...
// Đóng tất cả các modal custom (dùng cho list-kit.html và các trang staff)
function closeAllCustomModals() {
  document.querySelectorAll('.payment-error-modal, .payment-warning-modal, .confirm-modal, .error-modal, .modal').forEach(modal => modal.remove());
}
window.closeAllCustomModals = closeAllCustomModals;
